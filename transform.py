"""
Code reference: https://github.com/chijames/Poly-Encoder/blob/master/transform.pyhttps://github.com/chijames/Poly-Encoder/blob/master/transform.py
"""
...
class SelectionSequentialTransform(object):
    """
    1. 입력된 텍스트 토크나이즈
    2. input_ids와 input_masks 에 대해 리스트에 추가 후 리턴
    """
...
class SelectionConcatTransform(object):
    """
    크로스 인코더에서 사용하는 경우 context와 response 문장을 concat해서 입력으로 사용한다.
    """
    def __init__(self, tokenizer, max_len):
        self.tokenizer = tokenizer
    ...
    def __call__(self, context, responses):
        ...
        context = '\n'.join(context)
        tokenized_dict = self.tokenizer.encode_plus(context)
        context_ids, context_masks, context_segment_ids = tokenized_dict['input_ids'], tokenized_dict['attention_mask'], \
        ...
        for response in responses:
            ...
            response_ids, response_masks, response_segment_ids = tokenized_dict['input_ids'], tokenized_dict[
                'attention_mask'], tokenized_dict['token_type_ids']
